        ...
        initCalendar: function () {
			...
			var add_schedule = function (title) {
				...
				//	console.log(post_data);
					...
					/* 添加数据 */				
					var result = ajax_post_setup('?s=/Admin/SproomSchedule/Ajax_room_schedule_edit',post_data);
					if (result == false) {
						alert('服务器连接超时');
						return false;
			...
			var calendar_init = { //re-initialize the calendar
           	//	 contentHeight: 1024,	
				...
				events : function (start, end, callback) {
					...
					var result = ajax_post_setup('?s=/Admin/SproomSchedule/AJAX_Get_Schedule',{'hotel_room_id':hotel_room_id});
					var event_data = [];			
					if (result.status == 0) {
						//现付价格
				...
				eventClick:function( event, jsEvent, view ) { 
					...
					if (confirm('确定要删除吗？') == true) {
						var id = event.id;
						var result = ajax_post_setup('?s=/Admin/SproomSchedule/AJAX_DEL_Schedule',{'id':id});
						if (result == false) {
							alert('服务器连接超时');
							return false;
						} else if (result.status == 0) {
