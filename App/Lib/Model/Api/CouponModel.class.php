...
class CouponModel extends ApiBaseModel{
   ...
   public function get_coupon($type){
      ...
      $data = $this->where(array('is_del'=>0,'status'=>0,'over_time'=>array('GT',strtotime(date('Y-m-d',time()))),'number'=>array('GT',0),'type'=>$type))->select();
	  $i =1;
...
//获取用户的优惠券
   public function get_user_coupon($user_code){
	   ...
	   $data = $this->field('*')
		   ...
		   ->where(array('uc.wxid'=>"$user_code",'c.is_del'=>0,'uc.is_del'=>0))
		   ->select();
	   //return array();
		$i =1;
		foreach($data as $key=>$val){
