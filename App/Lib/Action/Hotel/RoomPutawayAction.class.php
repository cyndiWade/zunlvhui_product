 ...
 * 房间下架管理
 */
class RoomPutawayAction extends HotelBaseAction {
	...
	public function index () {
		...
		//连接数据库表
		$RoomPutaway = $this->db['RoomPutaway'];		//酒店房型表
	...
	public function putaway_edit () {
		...
		$RoomPutaway = $this->db['RoomPutaway'];		//下架日期表
		...
		if ($act == 'add') {								//添加
			if ($this->isPost()) {
				...
				$RoomPutaway->add_one_data() ? $this->success('添加成功！',U('Admin/RoomPutaway/index',array('hotel_room_id'=>$hotel_room_id))) : $this->error('添加失败请重新尝试！');
				exit;
			}
